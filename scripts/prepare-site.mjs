...
@media (max-width:767px){.custom-css header.menu img[src="/images/logo.png"]{max-width:92px!important;max-height:28px!important;width:auto!important;height:auto!important}}
  ...
  if (!html.includes('class="moonstone-support-band"')) {
    html = html.replace(/<footer\b/i, `${moonstoneSharedSections}<footer`);
  }
  ...
  html = html.replace(/href="\/(?:blog|industry-insights-legal-videos-library)\/[^"]*"/g, 'href="/content-hub/"');
  html = html.replace(/<a([^>]*?)href="[^"]*"([^>]*\btitle="Videos"[^>]*)>\s*Videos\s*<\/a>/gi, '<a$1href="/content-hub/"$2>Recent Updates</a>');
  html = html.replace(/<a([^>]*?)href="[^"]*"([^>]*\btitle="Blog"[^>]*)>\s*Blog\s*<\/a>/gi, '<a$1href="/content-hub/"$2>Legal Briefings</a>');
  html = html.replace(/<a([^>]*?)href="[^"]*"([^>]*\btitle="Podcast"[^>]*)>\s*Podcast\s*<\/a>/gi, '<a$1href="/reviews/"$2>Client Care</a>');
...
const styledPage = ({ title, intro, items, type }) => {
  ...
  const itemList = items.map((item) => {
    if (typeof item === 'string') return `<li id="${slugify(item)}">${escapeHtml(item)}</li>`;
...
${isLandingPage ? '' : `<section class="py-20 relative overflow-hidden bg-base-light">
  ...
  return `${beforeBanner}${banner}${middle}${moonstoneSharedSections}${template.footerAndAfter}`;
};
...
const notFoundPage = editorialPage({
  title: 'Page Not Found',
  intro: 'The page you requested could not be found.',
  content: `<section class="py-20 bg-base-light"><div class="moonstone-not-found px-4 content"><span class="moonstone-eyebrow">Error 404</span><h2>We Could Not Find That Page</h2><p>The address may have changed or the link may be incorrect. Use one of the options below to continue.</p><div class="moonstone-not-found-actions"><a class="btn btn-primary" href="/">Return home</a><a class="btn" href="/services/">Explore services</a><a class="btn" href="/contact/">Contact us</a></div></div></section>`
});
writeFileSync(path.join(target, '404.html'), notFoundPage);
...
for (const file of finalHtmlFiles) {
  ...
  const isNotFoundPage = relativePath.toLowerCase() === '404.html';
  const route = isNotFoundPage ? '/404/' : relativePath === 'index.html' ? '/' : `/${relativePath.replace(/index\.html$/, '')}`;
  ...
  const verification = process.env.GOOGLE_SITE_VERIFICATION
    ...
    : '';
  const robotsDirective = isNotFoundPage ? 'noindex, follow' : 'index, follow, max-image-preview:large, max-snippet:-1, max-video-preview:-1';
